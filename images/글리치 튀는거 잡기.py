...
class AutoGIFBackgroundRemover:
    ...
    def smart_flood_fill(self, img, tolerance):
        ...
        # (이미지가 모서리까지 꽉 찬 경우가 아니면, 모서리는 무조건 배경이라고 가정합니다.)
        ...
        while queue:
            ...
            # 이미 투명하거나, 캐릭터 윤곽선(다른 색)을 만났다면 중단
            if a == 0:
                continue

            # 색상 일치 여부 확인 (오차 범위 감안)
            # FFBE GIF는 배경색도 완벽하지 않고 미세한 노이즈가 있어서 오차가 필요합니다.
            if (abs(r - target_r) <= tolerance and
                abs(g - target_g) <= tolerance and
                abs(b - target_b) <= tolerance):
                
                # [매치!] 배경색으로 판단되면 투명으로 밀어버립니다.
                pixels[x, y] = (0, 0, 0, 0)
                
                # 현재 픽셀 주변 4방향 (상, 하, 좌, 우) 픽셀을 탐색 큐에 추가
                # (연결된 배경을 타고 들어갑니다.)
                for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                    next_x, next_y = x + dx, y + dy
                    if not (next_x, next_y) in processed:
